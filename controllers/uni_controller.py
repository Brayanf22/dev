...
def get_universidades():
    ...
    return jsonify([{'id': u.id, 'nombre': u.nombre, 'codigo': u.codigo} for u in universidades]), 200

@uni_bp.route('/uni/<int:uni_id>', methods=['GET'])
def get_universidad(uni_id):
    uni = service.obtener_uni(uni_id)
    if uni:
        return jsonify({'id': uni.id, 'nombre': uni.nombre, 'codigo': uni.codigo}), 200
...
def create_universidad():
    ...
    uni = service.crear_uni(nombre, codigo)
    return jsonify({'id': uni.id, 'nombre': uni.nombre, 'codigo': uni.codigo}), 201
...
def update_universidad(uni_id):
    ...
    nombre = data.get('nombre')
    codigo = data.get('codigo')
    uni = service.actualizar_uni(uni_id, nombre, codigo)
    if uni:
        return jsonify({'id': uni.id, 'nombre': uni.nombre, 'codigo': uni.codigo}), 200
    return jsonify({'error': 'Universidad no encontrada'}), 404
